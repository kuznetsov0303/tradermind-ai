...
$raw=Join-Path $Audit "S10_6S3_DAY7D_OFFLINE_COORDINATOR_raw_$stamp.json"
$report=Join-Path $Audit "S10_6S3_DAY7D_OFFLINE_COORDINATOR_report_$stamp.txt"
$generationFile=Join-Path $State "dynamic_universe_generation_preview_v1.json"
$localSh=Join-Path $env:TEMP "s10_6s3_offline_coordinator_$stamp.sh"
$remoteSh="/tmp/s10_6s3_offline_coordinator_$stamp.sh"
...
WORK="/tmp/s10_6s3_offline_coordinator"
rm -rf "$WORK"
mkdir -p "$WORK"
...
class OfflineCoordinatorTests(unittest.TestCase):
    ...
    def test_legitimate_core_symbols_with_r_or_u_are_allowed(self):
        generation=build_generation(*self.fixtures())
        self.assertIn("UBER",generation["desiredSymbols"])
        self.assertIn("MSTR",generation["desiredSymbols"])
...
$generation = $null
if($props.ContainsKey("generation")){
  $generation = $props["generation"]
  if($null -ne $generation){
    $generation | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $generationFile -Encoding UTF8
  }
}

$ok = [bool]($props["ok"])
$classification = [string]($props["classification"])
$compileRc = if($props.ContainsKey("compileReturnCode")){ $props["compileReturnCode"] } else { $null }
$testRc = if($props.ContainsKey("testReturnCode")){ $props["testReturnCode"] } else { $null }
$runRc = if($props.ContainsKey("runReturnCode")){ $props["runReturnCode"] } else { $null }
$deterministic = if($props.ContainsKey("deterministic")){ $props["deterministic"] } else { $null }
$validationErrors = if($props.ContainsKey("validationErrors")){ @($props["validationErrors"]) } else { @() }
$streamChanged = if($props.ContainsKey("streamSymbolsChanged")){ $props["streamSymbolsChanged"] } else { $false }
$systemdTouched = if($props.ContainsKey("systemdTouched")){ $props["systemdTouched"] } else { $false }
$nextAction = if($props.ContainsKey("nextAction")){ $props["nextAction"] } else { "FIX_OFFLINE_COORDINATOR_IN_ISOLATION" }
$compileOutput = if($props.ContainsKey("compileOutput")){ [string]$props["compileOutput"] } else { "" }
$testOutput = if($props.ContainsKey("testOutput")){ [string]$props["testOutput"] } else { "" }
$runtimeError = if($props.ContainsKey("runtimeError")){ [string]$props["runtimeError"] } else { "" }
...
if($null -ne $generation){
  $gprops = @{}
  foreach($p in $generation.PSObject.Properties){ $gprops[$p.Name] = $p.Value }
  if($gprops.ContainsKey("generationId")){ $generationId = [string]$gprops["generationId"] }
  if($gprops.ContainsKey("capacity")){ $capacity = $gprops["capacity"] }
  if($gprops.ContainsKey("desiredSymbols")){ $desired = @($gprops["desiredSymbols"]) }
  if($gprops.ContainsKey("addedSymbols")){ $added = @($gprops["addedSymbols"]) }
  if($gprops.ContainsKey("removedSymbols")){ $removed = @($gprops["removedSymbols"]) }
}

@(
 "S10.6S3 DAY 7D OFFLINE COORDINATOR",
 "Generated=$stamp",
 "OK=$ok",
 ...
 "CAPACITY=$capacity",
 "DESIRED_SYMBOL_COUNT=$($desired.Count)",
 "ADDED_SYMBOLS=$($added -join ',')",
 "REMOVED_SYMBOLS=$($removed -join ',')",
 "VALIDATION_ERRORS=$($validationErrors -join ',')",
 "STREAM_SYMBOLS_CHANGED=$streamChanged",
 "SYSTEMD_TOUCHED=$systemdTouched",
...
$milestone=Join-Path $Milestones "S10_6S3_DAY7D_OFFLINE_COORDINATOR_$stamp.md"
@"
# S10.6S3 Day 7D Offline Coordinator
...
- OK: $ok
- Classification: $classification
- Compile RC: $compileRc
- Test RC: $testRc
- Run RC: $runRc
- Deterministic: $deterministic
- Generation ID: $generationId
- Capacity: $capacity
- Desired symbols: $($desired.Count)
- Added: $($added -join ', ')
...
Write-Host ""
Write-Host "=== S10.6S3 DAY 7D OFFLINE COORDINATOR COMPLETE ===" -ForegroundColor Green
Write-Host "OK: $ok"
...
Write-Host "Desired symbol count: $($desired.Count)"
Write-Host "Added symbols: $($added -join ', ')"
Write-Host "Removed symbols: $($removed -join ', ')"
Write-Host "Validation errors: $($validationErrors -join ', ')"
Write-Host "Stream symbols changed: $streamChanged"
Write-Host "Systemd touched: $systemdTouched"
Write-Host "Next action: $nextAction"
Write-Host "Generation preview: $generationFile"
Write-Host "Report: $report"
Write-Host "Raw: $raw"
Write-Host "Milestone: $milestone"
if($compileOutput){ Write-Host "Compile output:"; Write-Host $compileOutput }
if($testOutput){ Write-Host "Test output:"; Write-Host $testOutput }
if($runtimeError){ Write-Host "Runtime error:"; Write-Host $runtimeError }
