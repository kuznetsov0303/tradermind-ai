...
# Remove the language dropdown state introduced by S10.9D.
text = text.replace(
...
change_language_block = '''  const changeLanguage = (nextLanguage: Language) => {
...
if change_language_block in text:
    text = text.replace(change_language_block, '', 1)

dropdown_start = '''      <div className="relative">
        <button
          type="button"
...
start_index = text.find(dropdown_start)
end_index = text.find(back_to_site_anchor)

if start_index == -1 or end_index == -1 or end_index <= start_index:
    restore()
    raise SystemExit("Injected dashboard language dropdown block not found")
...
for forbidden in (
    ...
    if forbidden in text:
        restore()
        raise SystemExit(f"Dashboard language dropdown residue remains: {forbidden}")
