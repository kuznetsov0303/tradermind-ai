...
import { getRequiredServerEnvFrom } from "@/lib/security/server-env";
...
const DEFAULT_LOOKBACK_HOURS = 96;
const DEFAULT_MAX_ALERTS = 120;
const DEFAULT_HORIZON_HOURS = 8;
const MAX_OUTCOME_ALERTS_PER_RUN = 150;

function getOutcomeCronSecrets() {
  return Array.from(
    new Set(
      [process.env.CRON_SECRET, process.env.OUTCOME_CRON_SECRET, process.env.MARKET_ALERTS_CRON_SECRET]
...
function isAuthorizedCronRequest(request: Request) {
  const secrets = getOutcomeCronSecrets();
...
async function fetchFmpCandles(symbol: string, startIso: string): Promise<Candle[]> {
  const apiKey = getRequiredServerEnvFrom(
    ["FMP_API_KEY", "FINANCIAL_MODELING_PREP_API_KEY"],
    "FMP 5m stock candles"
  );
...
function analyzeOutcome(
  ...
  for (const candle of candles) {
    ...
    // If the stop is hit before any target, the signal is resolved as stopped and
    // later candles must not turn it into a false TP hit.
    const hadTargetBeforeThisCandle = Boolean(target1HitAt || target2HitAt || target3HitAt);
    ...
    if (!stopHitAt && stopHitThisCandle) {
      ...
      events.push({
        ...
        metadata: {
          conservative: true,
          sameCandleStopFirst:
            !hadTargetBeforeThisCandle &&
            (tp1HitThisCandle || tp2HitThisCandle || tp3HitThisCandle),
        },
      });

      if (!hadTargetBeforeThisCandle) {
        break;
      }
    }
...
async function loadPendingAlerts({
  ...
  const scanLimit = onlyAlertId ? 1 : Math.min(Math.max(limit * 6, 250), 500);
  ...
  const allowedStatuses = new Set(["pending", "entry_touched"]);

  return ((data || []) as MarketAlertRow[])
    .filter((alert) => {
      const status = String(alert.outcome_status || "").toLowerCase();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const limit = Math.min(Number(body.limit || DEFAULT_MAX_ALERTS), MAX_OUTCOME_ALERTS_PER_RUN);
    const lookbackHours = Math.min(
      Number(body.lookbackHours || body.scanHours || DEFAULT_LOOKBACK_HOURS),
      168
    );
...
export async function GET(request: Request) {
  ...
  if (isAuthorizedCronRequest(request)) {
    const limit = Math.min(Number(url.searchParams.get("limit") || DEFAULT_MAX_ALERTS), MAX_OUTCOME_ALERTS_PER_RUN);
    const lookbackHours = Math.min(
      Number(
        url.searchParams.get("lookbackHours") ||
          url.searchParams.get("scanHours") ||
