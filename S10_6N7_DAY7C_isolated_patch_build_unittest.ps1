...
$raw=Join-Path $Audit "S10_6N7_DAY7C_PATCH_BUILD_raw_$stamp.json"
$report=Join-Path $Audit "S10_6N7_DAY7C_PATCH_BUILD_report_$stamp.txt"
$localSh=Join-Path $env:TEMP "s10_6n7_day7c_patch_build_$stamp.sh"
$remoteSh="/tmp/s10_6n7_day7c_patch_build_$stamp.sh"
...
WORK="/tmp/s10_6n7_day7c_patch_$(date +%s)"
...
# Opening range becomes complete at/after 09:35 NY even when
# no new trade arrives exactly after the boundary.
replace_once(
    r'(?m)^(\s*)"complete":\s*bool\(opening\.get\("complete"\)\),\s*$',
    lambda m: (
        f'{m.group(1)}"complete": bool(\n'
        f'{m.group(1)}    opening.get("complete")\n'
        f'{m.group(1)}    or (\n'
        f'{m.group(1)}        opening.get("high") is not None\n'
        f'{m.group(1)}        and opening.get("low") is not None\n'
        f'{m.group(1)}        and generated.astimezone(NEW_YORK).time() >= time(9, 35)\n'
        f'{m.group(1)}    )\n'
        f'{m.group(1)}),'
    ),
    "opening range completion semantics",
)
...
Write-Host ""
Write-Host "=== UPLOAD DAY 7C ISOLATED PATCH BUILD V7 ===" -ForegroundColor Green
...
@(
 "S10.6N7 DAY 7C ISOLATED PATCH BUILD",
 "Generated=$stamp",
 "OK=$($r.ok)",
...
Write-Host ""
Write-Host "=== S10.6N7 DAY 7C PATCH BUILD COMPLETE ===" -ForegroundColor Green
Write-Host "OK: $($r.ok)"
