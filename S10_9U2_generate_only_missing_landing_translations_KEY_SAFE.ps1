...
def call_api(locale_code: str, locale_name: str, batch: dict) -> dict:
    instructions = (
        ...
        "Return only a valid JSON object with exactly the same short keys such as k001, k002. "
        "Never alter, extend, rename or reorder those keys. "
...
for locale_code, locale_name in target_locales.items():
    ...
    for start in range(0, len(all_keys), batch_size):
        ...
        short_to_real = {
            f"k{index + 1:03d}": real_key
            for index, real_key in enumerate(translatable_keys)
        }

        api_batch = {
            short_key: batch[real_key]
            for short_key, real_key in short_to_real.items()
        }

        translated = dict(protected_batch)
        ...
        if api_batch:
            ...
            expected_short_keys = set(short_to_real)
            returned_short_keys = set(api_result)

            if expected_short_keys != returned_short_keys:
                raise SystemExit(
                    f"Short-key validation failed for {locale_code}: "
                    f"missing={sorted(expected_short_keys-returned_short_keys)} "
                    f"extra={sorted(returned_short_keys-expected_short_keys)}"
                )

            translated.update({
                real_key: api_result[short_key]
                for short_key, real_key in short_to_real.items()
            })
