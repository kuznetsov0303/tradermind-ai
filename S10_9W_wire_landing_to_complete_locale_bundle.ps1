...
const functionPattern =
  /function getLandingDictionaryLocale\(locale: Locale\): "en" \| "ru" \| "ua" \{[\s\S]*?\n\}\n\n/;

if (!functionPattern.test(patchedLanding)) {
  restore();
  fail("getLandingDictionaryLocale function anchor not found");
}

patchedLanding = patchedLanding.replace(functionPattern, "");
...
const raw = {
  ...
  oldThreeLanguageSelectorRemoved: true,
  buildPassed: true,
...
fs.writeFileSync(
  ...
  [
    ...
    "OLD_THREE_LANGUAGE_SELECTOR_REMOVED=True",
    "BUILD_PASSED=True",
...
fs.writeFileSync(
  ...
  [
    ...
    "- Old 3-language fallback selector removed: True",
    "- Build passed: True",
...
console.log("Old 3-language selector removed: True");
