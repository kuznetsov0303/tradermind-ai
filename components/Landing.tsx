...
base.ru = {
  ...
  heroText:
    "TraderMind AI показывает, почему ты зарабатываешь, почему теряешь, какие паттерны стоит торговать и когда твоё поведение незаметно убивает PnL.",
  ...
  problemText:
    "Скриншоты, заметки, вотчлисты и эмоции разбросаны по разным местам. TraderMind AI собирает всё в одну систему для исполнения, разбора и поиска закономерностей.",
  problems: [
    ["Случайные победы кажутся навыком", "Ты помнишь крупные плюсы, но не видишь точные условия, из-за которых они сработали."],
    ["Ошибки повторяются тихо", "Поздние входы, revenge trades и слабые выходы возвращаются, потому что их никто не измеряет."],
    ["Нет live-контекста рынка", "Журнал знает сделку, но не знает, что делали похожие геперы и холты в этот же день."],
  ],
  ...
  features: [
    ["AI Pattern Engine", "Находит повторяемые преимущества по сетапу, времени, тикеру, гепу, объёму и поведению трейдера.", "brain"],
    ["Журнал сделок", "Фиксируй сделки с тегами, скриншотами, заметками, эмоциями, риском и результатом.", "chart"],
    ["Поиск ошибок", "Находит поздние входы, revenge trading, нарушение стопа, оверсайз и слабые выходы.", "risk"],
    ["AI-коуч", "Дает проверки до входа, разбор после сделки, недельные отчёты и советы.", "bot"],
    ["Разбор скриншотов", "Загружай графики и сравнивай структуру с лучшими и худшими примерами.", "camera"],
    ["Оценка дисциплины", "Отделяй торговый навык от поведения через измеримый discipline score.", "target"],
  ],
  liveEyebrow: "Рынок в реальном времени",
  liveTitle: "Скринер геперов и холтов с AI-поиском похожих ситуаций за день.",
  liveText:
    "Система следит за геперами, холтами, возобновлениями, float, relative volume и momentum. Когда пользователь загружает скрин, AI ищет похожие ситуации этой же сессии.",
  agentsEyebrow: "AI-агенты",
  agentsTitle: "Команда AI-агентов за спиной каждого трейдера.",
  agentsText:
    "Вместо одного обычного чат-бота TraderMind AI использует специализированных агентов для разбора, риска, рыночного контекста и поведенческого коучинга.",
...
base.uk = {
  ...
  heroText:
    "TraderMind AI показує, чому ти заробляєш, чому втрачаєш, які патерни варто торгувати і коли твоя поведінка непомітно вбиває PnL.",
