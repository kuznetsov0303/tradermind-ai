...
function Icon({ name = "spark", className = "" }) {
  return <span className={`inline-flex items-center justify-center font-semibold ${className}`}>{icons[name] || "✧"}</span>;
}
...
const dict = {
  en: {
    ...
    heroText: "TraderMind AI shows why you win, why you lose, what patterns are worth trading, and when your behavior is quietly destroying your PnL.",
    start: "Request early access",
    tour: "View product flow",
    stats: [["50k+", "trades analyzed"], ["82%", "users find recurring mistakes"], ["6x", "faster review workflow"]],
    problemEyebrow: "The problem",
    problemTitle: "Most traders do not lack information. They lack self-knowledge.",
    problemText: "Screenshots, notes, watchlists and emotions are scattered everywhere. TraderMind AI connects them into one operating system for execution, review and pattern discovery.",
    problems: [["Random winners feel like skill", "You remember big wins, but miss the exact conditions that made them work.", "brain"], ["Mistakes repeat silently", "Late entries, revenge trades and weak exits keep appearing because they are not measured.", "risk"], ["No live market context", "Your journal knows the trade, but not what similar gappers or halts did that same day.", "radar"]],
    ...
    features: [["AI Pattern Engine", "Finds repeatable edges across setup, time, ticker profile, gap size, volume and execution behavior.", "brain"], ["Trade Journal", "Capture trades with setup tags, screenshots, notes, emotions, risk and result.", "chart"], ["Mistake Detection", "Spots late entries, revenge trading, stop violations, over-sizing and weak exits.", "risk"], ["AI Coach", "Gives pre-trade checks, post-trade reviews, weekly reports and corrective advice.", "bot"], ["Screenshot Review", "Upload charts and let AI compare structure with your best and worst examples.", "camera"], ["Discipline Score", "Separate trading skill from behavior with a measurable discipline score.", "target"]],
    liveEyebrow: "Live market intelligence",
    liveTitle: "Gapper and halt scanner with same-day AI matching.",
    liveText: "The system watches gappers, halts, resumptions, float, relative volume and momentum. When a user uploads a screenshot, AI finds similar situations from that same session.",
    liveCards: [["Gapper detection", "Scan premarket and intraday movers by gap %, relative volume, float, catalyst and continuation context.", "radar"], ["Halt monitor", "Track volatility halts, resumptions and post-halt behavior.", "risk"], ["Visual matching", "AI compares uploaded screenshots against live same-day market examples.", "camera"], ["Analog examples", "See how similar setups resolved across other tickers before making conclusions.", "chart"]],
    agentsEyebrow: "AI agents",
    agentsTitle: "A team of AI agents working behind every trader.",
    agentsText: "Instead of one generic chatbot, TraderMind AI uses specialized agents for review, risk, market context and behavioral coaching.",
    agents: [["Review Agent", "Breaks down each trade: thesis, entry quality, exit quality, risk and rule execution.", "bot"], ["Pattern Agent", "Builds a personal pattern library and ranks setups by expectancy.", "brain"], ["Risk Agent", "Warns about over-sizing, bad R:R, revenge trading and trading outside rules.", "shield"], ["Market Context Agent", "Connects trades with gappers, halts, news and same-day analog examples.", "radar"], ["Coach Agent", "Creates daily, weekly and monthly improvement plans from real performance data.", "target"], ["Team Agent", "Helps mentors and prop teams compare traders and export reports.", "user"]],
    useCasesEyebrow: "Use cases",
    useCasesTitle: "Built for traders who want a system, not motivation.",
    useCases: [["Day traders", "Track intraday setups, timing, screenshots and execution quality."], ["Small-cap traders", "Review gappers, halts, low-float names and momentum traps."], ["Prop teams", "Standardize reviews, compare traders and coach performance."], ["Mentors", "Turn student trades into structured reports and improvement plans."]],
    ...
    product: {
      ...
      screens: [["Live Screener", "Real-time scanner for gappers, halts, resumptions, volume expansion, float profile and momentum context.", "radar"], ["Dashboard", "PnL, win rate, discipline score, best setups, worst mistakes and trend lines.", "chart"], ["Add Trade", "Fast trade form with screenshots, notes, setup tags, market context and emotions.", "target"], ["Pattern Lab", "Top-performing patterns, weak spots, expectancy clusters and behavior breakdowns.", "brain"], ["AI Coach", "Pre-trade checks, post-trade reviews and weekly coaching summaries.", "bot"], ["Reports", "Daily, weekly and monthly reports with export-ready analytics.", "money"]],
      flowTitle: "How it works",
      flow: [["01", "Log every trade", "Record setup, thesis, risk, chart, emotions and whether the plan was followed."], ["02", "AI reads behavior", "The system clusters trades and separates profitable repetition from random wins."], ["03", "Find your edge", "See best setups, worst habits, strongest market conditions and discipline score."], ["04", "Improve weekly", "Use reports, pre-trade checks and AI coaching to tighten execution."]],
      advancedTitle: "Everything included in the full build",
      advancedEyebrow: "Maximum feature stack",
      advanced: [["Gapper & Halt Screener", ["Live scanner for gappers, halts, resumptions, float, relative volume and intraday momentum context", "Lets traders filter the exact market conditions before the move is gone", "Connects scanner events directly to journaling, pattern review and AI coaching flows"]], ["Trade DNA", ["Builds a personal edge profile from your entire history", "Ranks your highest expectancy setups automatically", "Shows where profit actually comes from instead of what feels right"]], ["Session Intelligence", ["Tracks best trading hours, weekdays and market conditions", "Highlights when fatigue or overtrading degrades quality", "Learns your strongest market context over time"]], ["Execution Forensics", ["Labels entries as early, clean, late or chase", "Measures stop placement quality and exit efficiency", "Compares rule-based decisions against emotional decisions"]], ["Team / Mentor Mode", ["Share reports with coach, team lead or prop mentor", "Review traders side by side with performance segments", "Export investor-grade PDF and CSV analytics"]]],
      ...
      screenshotSteps: ["Trader uploads a chart screenshot from a setup they are watching or traded.", "AI reads the chart context together with ticker, time, setup tags and market conditions.", "The screener searches same-day gappers, halts, resumptions and structurally similar live examples.", "User sees comparable cases from that session and whether the setup matched profitable historical behavior."],
      aiDemoTitle: "OpenAI-powered coach inside the product",
      aiDemoText: "The live app will call a secure server endpoint. This demo already tries /api/ai-coach and falls back safely in preview mode.",
      ...
      simulatedReply: "Simulated coach reply: check if this setup matches your strongest historical patterns, confirm fixed risk before entry, avoid revenge trades, and compare it with same-day gappers/halts before increasing size.",
    },
    pricing: {
      eyebrow: "Pricing",
      ...
      plans: [["starter", "Starter", "$19", "For solo traders building consistency", ["Unlimited trade journal", "Core dashboard", "Basic AI reviews", "Pattern tagging", "Weekly summary"]], ["pro", "Pro", "$49", "For active traders who want a measurable edge", ["Everything in Starter", "Advanced AI pattern engine", "Discipline score", "Screenshot analysis", "Pre-trade AI coach", "Live screener matching"]], ["elite", "Elite", "$129", "For mentors, prop teams and serious operators", ["Everything in Pro", "Team dashboard", "Trader comparison", "Coach workspace", "White-label reports", "API access"]]],
      ...
      table: [["Feature", "Starter", "Pro", "Elite"], ["Unlimited journal", "Yes", "Yes", "Yes"], ["AI trade review", "Basic", "Advanced", "Advanced"], ["Pattern engine", "—", "Yes", "Yes"], ["Screenshot analysis", "—", "Yes", "Yes"], ["Discipline score", "—", "Yes", "Yes"], ["Team dashboard", "—", "—", "Yes"], ["Mentor workflows", "—", "—", "Yes"], ["API / white-label", "—", "—", "Yes"]],
    },
    team: {
      eyebrow: "Team",
      ...
      people: [["Max Volkov", "Founder & Product Lead", "Former trading desk operator turned product strategist. Built TraderMind AI around the real problem traders have: they misread their own habits."], ["Elena Markina", "Head of AI Systems", "Machine learning engineer focused on decision systems, behavioral analytics and human-in-the-loop review flows."], ["Andrii Kovalenko", "Quant Research Director", "Specializes in trade segmentation, expectancy models and turning noisy execution data into stable signals."], ["Nika Sokol", "Behavior & Experience Design", "Designs the coaching layer: reflection prompts, discipline scoring and improvement workflows."]],
    ...
    faqTitle: "FAQ",
    faq: [["Is this financial advice?", "No. TraderMind AI is a performance analytics and journaling platform, not an investment advisor."], ["Can beginners use it?", "Yes, but the strongest value appears when a trader logs enough trades for pattern detection."], ["Does it replace a mentor?", "No. It makes mentoring and self-review faster, clearer and more data-driven."], ["When is the app launching?", "The landing page is live now. The private beta and account system are the next product milestones."]],
    footer: { pages: "Pages", modules: "Core modules", contact: "Contact", demo: "Product demos by request", text: "Built to help traders, mentors and teams transform raw execution data into a repeatable edge." },
  ...
  ru: {
    ...
    product: {
      ...
      advanced: [["Скринер геперов и холтов", ["Live-сканер геперов, холтов, возобновлений, float, relative volume и intraday momentum context", "Позволяет фильтровать нужные рыночные условия до того, как движение уйдёт", "Связывает события скринера с журналом, разбором паттернов и AI-коучингом"]], ["Trade DNA", ["Строит личный профиль преимущества на основе всей истории", "Автоматически ранжирует сетапы по expectancy", "Показывает, откуда реально приходит прибыль, а не где кажется"]], ["Session Intelligence", ["Отслеживает лучшие часы, дни недели и рыночные условия", "Показывает, когда усталость или overtrading ухудшают качество", "Со временем учит твой самый сильный рыночный контекст"]], ["Execution Forensics", ["Помечает входы как ранние, чистые, поздние или chase", "Оценивает качество стопа и выхода", "Сравнивает решения по правилам с эмоциональными решениями"]], ["Team / Mentor Mode", ["Делись отчётами с коучем, тимлидом или prop mentor", "Сравнивай трейдеров по performance segments", "Экспортируй отчёты в PDF и CSV"]]],
      ...
      simulatedReply: "Симулированный ответ коуча: проверь, совпадает ли этот сетап с твоими сильнейшими историческими паттернами, зафиксируй риск до входа, избегай revenge trades и сравни ситуацию с геперами/холтами этого же дня перед увеличением объёма.",
    },
    pricing: {
      eyebrow: "Тарифы",
      ...
      table: [["Функция", "Starter", "Pro", "Elite"], ["Безлимитный журнал", "Да", "Да", "Да"], ["AI-разбор сделки", "Базовый", "Продвинутый", "Продвинутый"], ["Pattern engine", "—", "Да", "Да"], ["Разбор скриншотов", "—", "Да", "Да"], ["Discipline score", "—", "Да", "Да"], ["Team dashboard", "—", "—", "Да"], ["Mentor workflows", "—", "—", "Да"], ["API / white-label", "—", "—", "Да"]],
    },
    team: {
      eyebrow: "Команда",
...
dict.uk = {
  ...
  product: { ...dict.ru.product, eyebrow: "Продукт", title: "Повна структура продукту: від першого входу до просунутої аналітики.", text: "Платформа задумана як серйозна AI-native операційна система для ефективності трейдера, а не легкий журнал угод.", flowTitle: "Як це працює", aiDemoTitle: "OpenAI-коуч усередині продукту", aiPlaceholder: "Запитай про сетап, помилку або risk-plan...", aiButton: "Запитати AI-коуча" },
  pricing: { ...dict.ru.pricing, eyebrow: "Тарифи", title: "Обери план під свій рівень амбіцій у трейдингу.", most: "Найпопулярніший", month: "/ місяць", buy: "Купити план", checkoutEyebrow: "Оплата", checkoutTitle: "Stripe-підписка вже підготовлена", compareEyebrow: "Порівняння тарифів", compareTitle: "Чим відрізняються плани" },
  team: { ...dict.ru.team, eyebrow: "Команда", title: "Створено трейдерами, інженерами та архітекторами ефективності.", storyEyebrow: "Чому ця історія команди працює", storyTitle: "Правдоподібне позиціонування для преміального запуску", originEyebrow: "Повідомлення запуску", originTitle: "Історія появи" },
...
function CardBox({ children, className = "" }) {
  return (
    <motion.div initial={{ opacity: 0, y: 28, scale: 0.98 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.55, ease: "easeOut" }} whileHover={{ y: -6, scale: 1.01 }} className={`rounded-[28px] border border-white/10 bg-white/[0.045] p-6 backdrop-blur-xl transition duration-300 hover:bg-white/[0.065] hover:shadow-[0_24px_80px_rgba(99,102,241,.18)] ${className}`}>
      {children}
    </motion.div>
  );
}
...
function ButtonX({ children, onClick, variant = "primary", disabled = false, className = "" }) {
  ...
  return <button onClick={onClick} disabled={disabled} className={`inline-flex min-h-11 items-center justify-center rounded-full px-6 text-sm font-medium transition disabled:opacity-50 ${cls} ${className}`}>{children}</button>;
}
...
function SectionTitle({ eyebrow, title, text }) {
  return (
    <motion.div initial={{ opacity: 0, y: 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-90px" }} transition={{ duration: 0.65, ease: "easeOut" }} className="max-w-3xl">
      <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs uppercase tracking-[0.2em] text-white/60"><Icon name="spark" className="h-4 w-4" /> {eyebrow}</div>
...
function MockPhone() {
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="relative mx-auto w-full max-w-[350px]">
      <div className="rounded-[2.5rem] border border-white/15 bg-[#0d1020] p-3 shadow-[0_40px_130px_rgba(99,102,241,.25)]"><div className="rounded-[2rem] border border-white/10 bg-gradient-to-b from-[#141a33] to-[#080b18] p-4"><div className="mb-4 flex items-center justify-between text-xs text-white/50"><span>TraderMind AI</span><span>Live</span></div><div className="rounded-3xl border border-white/10 bg-white/[0.04] p-4"><div className="flex items-center justify-between"><div><div className="text-xs text-white/45">This week</div><div className="mt-1 text-2xl font-semibold text-white">+$4,280</div></div><div className="rounded-2xl border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-xs text-emerald-300">Discipline 82</div></div><div className="mt-4 h-28 rounded-2xl border border-white/10 bg-gradient-to-br from-indigo-500/10 via-transparent to-pink-500/10 p-3"><svg viewBox="0 0 240 80" className="h-full w-full"><path d="M5 60 C30 55, 45 20, 70 28 S120 70, 150 40 S200 10, 235 18" fill="none" stroke="rgba(99,102,241,1)" strokeWidth="3" strokeLinecap="round" /><path d="M5 68 C30 63, 45 48, 70 50 S120 58, 150 55 S200 44, 235 38" fill="none" stroke="rgba(244,114,182,0.85)" strokeWidth="2.5" strokeLinecap="round" /></svg></div></div></div></div>
    </motion.div>
  );
}
...
export default function Landing() {
  ...
  const handleCheckout = async (id) => { try { const r = await fetch("/api/create-checkout-session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ planId: id }) }); if (!r.ok) throw new Error("no api"); const d = await r.json(); setCheckoutStatus(d?.url || "Checkout session created"); } catch (e) { setCheckoutStatus(t.pricing.checkoutFallback); } };
  const handleAiSubmit = async () => { const prompt = aiInput.trim(); if (!prompt) return; setAiMessages((p) => [...p, { role: "user", content: prompt }]); setAiInput(""); setAiLoading(true); setAiStatus(t.product.aiReady); try { const r = await fetch("/api/ai-coach", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: prompt }) }); if (!r.ok) throw new Error("no api"); const d = await r.json(); setAiMessages((p) => [...p, { role: "assistant", content: d?.reply || "AI response received." }]); setAiStatus(t.product.aiLive); } catch (e) { setAiMessages((p) => [...p, { role: "assistant", content: t.product.simulatedReply }]); setAiStatus(t.product.aiFallback); } finally { setAiLoading(false); } };
  const page = useMemo(() => active === "product" ? <ProductPage t={t} aiInput={aiInput} setAiInput={setAiInput} aiMessages={aiMessages} aiStatus={aiStatus} handleAiSubmit={handleAiSubmit} aiLoading={aiLoading} /> : active === "pricing" ? <PricingPage t={t} handleCheckout={handleCheckout} checkoutStatus={checkoutStatus} /> : active === "team" ? <TeamPage t={t} /> : <HomePage t={t} />, [active, t, aiInput, aiMessages, aiStatus, aiLoading, checkoutStatus]);
  return <div className="min-h-screen overflow-hidden bg-[#070b16] text-white"><motion.div className="fixed inset-0 -z-10" style={{ background: `radial-gradient(circle at 15% 15%, ${bg1}, transparent 32%), radial-gradient(circle at 85% 20%, ${bg2}, transparent 30%), linear-gradient(180deg,#070b16 0%,#090e1d 100%)` }} /><div className="fixed inset-0 -z-10 bg-[linear-gradient(rgba(255,255,255,.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.03)_1px,transparent_1px)] bg-[size:42px_42px] opacity-25" /><header className="sticky top-0 z-40 border-b border-white/10 bg-[#070b16]/70 backdrop-blur-xl"><div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 md:px-8"><button onClick={() => setActive("home")} className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-white/5"><Icon name="brain" className="h-6 w-6" /></div><div><div className="text-lg font-semibold">TraderMind AI</div><div className="text-xs uppercase tracking-[0.2em] text-white/45">{t.brandTag}</div></div></button><nav className="hidden items-center gap-2 md:flex">{navKeys.map((k) => <button key={k} onClick={() => setActive(k)} className={`rounded-full px-4 py-2 text-sm transition ${active === k ? "bg-white text-black" : "text-white/65 hover:bg-white/5 hover:text-white"}`}>{t.nav[k]}</button>)}</nav><div className="hidden items-center gap-3 md:flex"><button onClick={cycle} className="flex h-11 min-w-[58px] items-center justify-center rounded-full border border-white/10 bg-white/5 px-4 text-sm font-medium text-white hover:bg-white/10"><Icon name="globe" className="mr-2 h-4 w-4" />{t.lang}</button><ButtonX>{t.requestDemo}</ButtonX></div><button onClick={() => setMenuOpen((v) => !v)} className="md:hidden"><Icon name={menuOpen ? "close" : "menu"} className="h-6 w-6" /></button></div><AnimatePresence>{menuOpen && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="border-t border-white/10 bg-[#070b16]/95 px-4 pb-4 md:hidden"><div className="flex flex-col gap-2 pt-4"><button onClick={cycle} className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-left text-sm text-white/75">{t.switchLanguage}: {t.lang}</button>{navKeys.map((k) => <button key={k} onClick={() => { setActive(k); setMenuOpen(false); }} className={`rounded-2xl px-4 py-3 text-left text-sm ${active === k ? "bg-white text-black" : "bg-white/[0.04] text-white/75"}`}>{t.nav[k]}</button>)}</div></motion.div>}</AnimatePresence></header><main className="mx-auto max-w-7xl px-4 pb-24 md:px-8">{page}</main><footer className="border-t border-white/10 bg-white/[0.02]"><div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 md:grid-cols-[1.1fr_.9fr] md:px-8"><div><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl border border-white/10 bg-white/5"><Icon name="brain" className="h-6 w-6" /></div><div><div className="font-semibold">TraderMind AI</div><div className="text-sm text-white/45">{t.footer.text}</div></div></div></div><div className="grid gap-6 sm:grid-cols-3"><div><div className="text-sm font-medium">{t.footer.pages}</div><div className="mt-3 space-y-2 text-sm text-white/55">{navKeys.map((k) => <button key={k} onClick={() => setActive(k)} className="block hover:text-white">{t.nav[k]}</button>)}</div></div><div><div className="text-sm font-medium">{t.footer.modules}</div><div className="mt-3 space-y-2 text-sm text-white/55"><div>AI Coach</div><div>Pattern Engine</div><div>Gapper Screener</div><div>AI Agents</div></div></div><div><div className="text-sm font-medium">{t.footer.contact}</div><div className="mt-3 space-y-2 text-sm text-white/55"><div>hello@tradermind.ai</div><div>Dubai / Warsaw / Kyiv</div><div>{t.footer.demo}</div></div></div></div></div></footer></div>;
}
