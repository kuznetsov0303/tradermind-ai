...
Write-Host "`n=== S10.4N VPS BACKUP / LEGACY / RETENTION AUDIT ===" -ForegroundColor Green
...
def extract_runtime_paths(units):
    ...
    for u in units:
        txt = u["content"]
        for m in re.findall(r'(/opt/skilledge/stock-engine/[^\s"\']+)', txt):
            refs.add(m.rstrip(";"))
    return sorted(refs)
...
for base, dirs, files in os.walk(ROOT):
    rel_parts = Path(base).relative_to(ROOT).parts if Path(base) != ROOT else ()
...
# Also inspect systemd text separately.
legacy_systemd_refs = []
for u in units:
    if legacy_target.name in u["content"] or str(legacy_target) in u["content"]:
        legacy_systemd_refs.append(u["name"])
...
# Current app.py and backups specifically
...
for p in (ROOT / "app/api").glob("app.py.bak*"):
    try:
        app_backups.append({
            ...
            "sha256": sha256_file(p),
            "sameAsCurrent": sha256_file(p) == app_current_hash if app_current_hash else False,
            "directRuntimeRef": str(p) in runtime_refs,
        })
    except Exception:
        pass

# Keep newest N backups by mtime as rollback candidates
app_backups_sorted = sorted(app_backups, key=lambda x: x["mtime"], reverse=True)
for i, rec in enumerate(app_backups_sorted):
    rec["retentionClass"] = "KEEP_ROLLBACK_RECENT" if i < 3 else "DELETE_CANDIDATE_AFTER_SNAPSHOT"
...
payload = {
    ...
    "backupCandidates": sorted(backup_candidates, key=lambda x: x["path"]),
...
$tempRemote = "/tmp/s10_4n_vps_cleanup_audit_$stamp.py"
$escaped = $remotePython.Replace("'", "'\''")
...
Write-Host "`n=== 1. RUN READ-ONLY REMOTE INVENTORY ===" -ForegroundColor Green

& ssh @sshArgs "cat > '$tempRemote' <<'PYEOF'
$remotePython
PYEOF
python3 '$tempRemote'
rm -f '$tempRemote'"
...
if ($LASTEXITCODE -ne 0) {
    throw "Remote audit command failed with exit code $LASTEXITCODE"
}

# Re-run with output capture cleanly.
$remoteCommand = @"
python3 - <<'PYEOF'
$remotePython
PYEOF
...
$jsonText = & ssh @sshArgs $remoteCommand

if ($LASTEXITCODE -ne 0) {
    throw "Remote JSON audit failed with exit code $LASTEXITCODE"
}

if ([string]::IsNullOrWhiteSpace(($jsonText -join "`n"))) {
    throw "Remote audit returned empty output."
}

$jsonJoined = $jsonText -join "`n"
$jsonJoined | Set-Content -LiteralPath $rawPath -Encoding UTF8

$data = $jsonJoined | ConvertFrom-Json

Write-Host "`n=== 2. SUMMARIZE ===" -ForegroundColor Green
...
$keepRollback = @($data.appBackups | Where-Object { $_.retentionClass -eq "KEEP_ROLLBACK_RECENT" })
$deleteAfterSnapshot = @($data.appBackups | Where-Object { $_.retentionClass -eq "DELETE_CANDIDATE_AFTER_SNAPSHOT" })
...
foreach ($b in @($data.appBackups)) {
    $lines.Add("$($b.retentionClass) | $($b.rel) | size=$($b.size) | sha256=$($b.sha256) | sameAsCurrent=$($b.sameAsCurrent) | directRuntimeRef=$($b.directRuntimeRef)")
}
