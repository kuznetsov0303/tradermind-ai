...
$localChecks=[ordered]@{
  ...
  executionContainsOnlyMarketStreamService=(($execution -match 'skilledge-market-stream.service') -and ($execution -notmatch 'skilledge-stock-engine-api.service'))
  ...
  executionContainsRampStages=($execution -match 'for CAP in 50 100 150 250')
  executionContainsFiveMinuteMicroSoak=(($execution -match 'seq 1 5') -and ($execution -match 'sleep 60'))
  executionContainsThirtyMinuteFinalSoak=($execution -match 'seq 1 30')
  executionContainsAutomaticRollback=(($execution -match 'ROLLBACK_STARTED') -and ($execution -match 'skilledge-market-stream.service.before'))
  rollbackContainsCore25Restore=($rollback -match 'skilledge-market-stream.service.before')
}
