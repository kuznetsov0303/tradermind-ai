...
'@
  ...
  $stdoutTemp=Join-Path $env:TEMP "s10_9x_build_stdout_$stamp.txt"
  $stderrTemp=Join-Path $env:TEMP "s10_9x_build_stderr_$stamp.txt"
  ...
  @"
COMMAND=$env:ComSpec /d /s /c npm run build
STATUS=$buildExit
...
"@ | Set-Content -LiteralPath $BuildLog -Encoding UTF8
  ...
  if($buildExit -ne 0){
    ...
    throw "Build failed with exit code $buildExit; Landing.tsx restored. See $BuildLog"
  }
