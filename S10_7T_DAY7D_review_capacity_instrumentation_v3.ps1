...
checks["activeQuoteSchemaCandidatesFound"]=bool(active_quote_candidates)
checks["quoteFreshnessSchemaOverlapProven"]=bool(quote_schema_overlap)

if not active_quote_candidates:
    warnings.append("ACTIVE_QUOTE_SCHEMA_CANDIDATES_NOT_FOUND_IN_SOURCE")
...
- Quote schema candidates: $(@($r.activeQuoteSchemaCandidates)-join ', ')
- Quote schema overlap: $(@($r.quoteSchemaOverlap)-join ', ')
...
Write-Host "Quote schema candidates: $(@($r.activeQuoteSchemaCandidates)-join ', ')"
Write-Host "Quote schema overlap: $(@($r.quoteSchemaOverlap)-join ', ')"
