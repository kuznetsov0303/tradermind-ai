...
$raw=Join-Path $Audit "S10_6N4_DAY7C_PATCH_BUILD_raw_$stamp.json"
$report=Join-Path $Audit "S10_6N4_DAY7C_PATCH_BUILD_report_$stamp.txt"
$localSh=Join-Path $env:TEMP "s10_6n4_day7c_patch_build_$stamp.sh"
$remoteSh="/tmp/s10_6n4_day7c_patch_build_$stamp.sh"
...
WORK="/tmp/s10_6n4_day7c_patch_$(date +%s)"
...
touch "$WORK/app/__init__.py" "$TESTS/__init__.py"
...
Write-Host ""
Write-Host "=== UPLOAD DAY 7C ISOLATED PATCH BUILD V4 ===" -ForegroundColor Green
...
@(
 "S10.6N4 DAY 7C ISOLATED PATCH BUILD",
 "Generated=$stamp",
 "OK=$($r.ok)",
...
Write-Host ""
Write-Host "=== S10.6N4 DAY 7C PATCH BUILD COMPLETE ===" -ForegroundColor Green
Write-Host "OK: $($r.ok)"
