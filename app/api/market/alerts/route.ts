...
function buildActionableTradePlan(params: {
  ...
  const entryMin = isShort ? price * 0.985 : price * 0.995;
  const entryMax = isShort ? price * 1.005 : price * 1.015;

  const stop = isShort ? price * 1.035 : price * 0.965;

  const target1 = isShort ? price * 0.965 : price * 1.035;
  const target2 = isShort ? price * 0.93 : price * 1.07;
  const target3 = isShort ? price * 0.88 : price * 1.12;
...
function buildAlertDraft(params: {
  ...
  const direction =
    alertType === "dump"
      ? "downside"
      : alertType === "social_spike" || alertType === "news_catalyst"
        ? "neutral"
        : "upside";

        const tradePlan = buildActionableTradePlan({
  alertType,
  direction,
  price,
  changePercent,
...
const setupEducation = getSetupEducation({
  alertType,
  direction,
  assetType,
  catalyst,
  ...
  const catalystBoost = catalyst ? 8 : 0;
  const socialBoost = Math.min(socialScore * 0.25, 18);
  const movementBoost = Math.min(Math.abs(changePercent), 18);

  const score = Math.round(
    clamp(marketScore * 0.68 + socialBoost + movementBoost + catalystBoost)
  );

  const confidenceScore = Math.round(
  clamp(
    score * 0.72 +
      Math.min(Math.abs(changePercent), 15) +
      (catalyst ? 6 : 0) +
      (params.social?.sources?.length ? 4 : 0)
  )
);
...
const signalPlaybook = buildPremiumSignalPlaybook({
  ...
  const titleMap: Record<AlertType, string> = {
    pump: `${symbol} pump / momentum expansion`,
    dump: `${symbol} dump / weakness expansion`,
    social_spike: `${symbol} social attention spike`,
    crypto_momentum: `${symbol} crypto momentum alert`,
    news_catalyst: `${symbol} fresh catalyst alert`,
  };
  ...
  if (catalyst) {
    reasonParts.push(`catalyst: ${catalyst}`);
  }

  const riskNote =
    params.row.risk_label ||
    (alertType === "dump"
      ? "Downside momentum can reverse fast after capitulation. Wait for confirmation."
      : "Momentum alert can be extended. Avoid chasing without structure confirmation.");

  const scenario =
    alertType === "dump"
      ? "Watch for failed reclaim, weak bounce, lower high, or breakdown continuation."
      : alertType === "crypto_momentum"
        ? "Watch liquidity sweep/reclaim behavior, continuation after displacement, and invalidation around structure."
        : "Watch VWAP/previous high reaction, pullback quality, volume continuation, and failed breakout risk.";

  return {
    alert_key: `${alertType}:${symbol}:${hourlyKey}`,
    user_id: null,
...
setup_confirmation: setupEducation.setup_confirmation,
setup_common_mistake: setupEducation.setup_common_mistake,
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(
    ...
    return NextResponse.json({
      source: period === "7d" ? "market_alerts_7d" : "market_alerts",
      period,
      count: data?.length || 0,
      items: data || [],
    });
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(
    ...
    if (marketResult.error) {
      console.error("Alert generator market load error:", marketResult.error);
    }
    ...
    const marketRows = pickBestMarketRows(
      ((marketResult.data || []) as MarketScannerRow[]).filter((row) =>
        normalizeSymbol(row.symbol || "")
      )
    );

    const socialBySymbol = aggregateSocial(
      ((socialResult.data || []) as SocialMentionRow[]).filter((row) =>
        normalizeSymbol(row.symbol || "")
      )
    );

    const drafts = marketRows
      .map((row) =>
        buildAlertDraft({
          row,
      ...
      .filter((item): item is MarketAlertDraft => Boolean(item))
    ...
    if (drafts.length > 0) {
      ...
      if (upsertError) {
        ...
        return NextResponse.json(
          { error: "Failed to save generated alerts." },
          { status: 500 }
        );
      }
    }
