...
function normalizeMover(item: FmpMover): NormalizedMover {
...
function isCorrectSide(item: NormalizedMover, side: MoverSide) {
...
export async function GET(request: Request) {
  try {
    ...
    const limit = Math.max(5, Math.min(50, Number.isFinite(rawLimit) ? rawLimit : 40));

    const minVolume = Number(process.env.CHART_STOCK_MOVERS_MIN_VOLUME || 100000);

    const rawMovers = await fetchFmpMovers(side);

    const baseCandidates = rawMovers
      .map(normalizeMover)
      .filter((item) => {
        if (!isProbablyUsStock(item.symbol)) return false;
        if (!Number.isFinite(item.changePct)) return false;
        if (!isCorrectSide(item, side)) return false;

        return true;
      });

    const strictVolumeCandidates = baseCandidates.filter((item) => {
      if (!item.hasReliableVolume) return false;
      return Number(item.rawVolume ?? 0) >= minVolume;
    });

    const candidates =
      strictVolumeCandidates.length > 0 ? strictVolumeCandidates : baseCandidates;
    ...
    const items = candidates
      ...
      .slice(0, limit)
      .map(({ rawVolume, hasReliableVolume, ...item }) => item);
    ...
    return NextResponse.json(
      {
        ...
        items,
        meta: {
          rawCount: rawMovers.length,
          baseCount: baseCandidates.length,
          strictVolumeCount: strictVolumeCandidates.length,
          usedVolumeFallback: strictVolumeCandidates.length === 0,
          minVolume,
