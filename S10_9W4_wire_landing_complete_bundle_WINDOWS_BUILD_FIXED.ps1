...
fs.writeFileSync(
  buildLog,
  [
    `COMMAND=${buildCommand.command} ${buildCommand.args.join(" ")}`,
    `STATUS=${String(build.status)}`,
    `SIGNAL=${String(build.signal)}`,
...
if (build.error || build.status !== 0) {
  ...
  const reason = build.error
    ? `Build process could not start: ${build.error.message}`
    : `Build exited with status ${build.status}`;

  fail(`${reason}; wiring was restored. See ${buildLog}`);
}
