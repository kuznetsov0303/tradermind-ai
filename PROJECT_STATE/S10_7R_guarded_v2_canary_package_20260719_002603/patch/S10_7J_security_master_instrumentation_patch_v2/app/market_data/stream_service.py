...
from .scanner import build_scanner_snapshot
...
class StreamRuntime:
    def __init__(self) -> None:
        ...
        try:
            if CANDLE_SNAPSHOT_PATH.exists():
                previous_candle_snapshot = json.loads(
                    CANDLE_SNAPSHOT_PATH.read_text(encoding="utf-8")
                )
                restored_symbols = self.candle_engine.hydrate_from_snapshot(
                    previous_candle_snapshot
                )
                LOGGER.info(
                    "candle snapshot hydration restored %s symbols",
                    restored_symbols,
                )
        except Exception:
            LOGGER.exception("candle snapshot hydration failed")
