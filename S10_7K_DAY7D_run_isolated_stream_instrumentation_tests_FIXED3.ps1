...
with tempfile.TemporaryDirectory(prefix="s10_7k_") as temp_raw:
    ...
    # Linux-only stdlib module used by production for process metrics.
    # Provide an in-memory Windows-compatible stub for isolated tests only.
    try:
        ...
        fake_resource.getrusage=lambda *_args,**_kwargs:FakeUsage()
